...
public class Log {
	...
	private Log kvOfCrypto(String key, Object value, ICryptoConvertor convertor) {
		if (isSansInfoCryptoEnable()) {
			params.put(key,
					convertor != null ? convertor.convert(value) : CryptoConvertConfig.getDefault(null).convert(value));
		} else {
			params.put(key, value);
		}
		return this;
	 ...
	 *            值
	 * @param convertorKey
	 *            在CryptoConvertConfig中注册的转换器key
	 * @return
	 */
	...
	public String toString() {
		return String.format(LOG_FORMAT, System.currentTimeMillis(), replaceSpecialChar(operation),
				replaceSpecialChar(params.toString()), replaceSpecialChar(message));
	}
