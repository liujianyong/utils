...
import com.liujy.utils.log.Log;
...
public class FtpClientService {
    ...
    public FTPClient getFtpClient() {
        ...
        try {
            ...
            LOGGER.error(Log.op("getFtpClient").msg("ftp login failure.").kv("user", ftpconfig.getUsername()).kv("password", ftpconfig.getPassword()).kv("message", e.getMessage()).toString(), e);
    ...
    public void closeFtpClient(FTPClient ftpClient) {
        if (ftpClient != null) {
            try {
                ...
                LOGGER.info(Log.op("getFtpClient").msg("ftp disconnects successfully.").toString());
            } catch (IOException e) {
                LOGGER.error(Log.op("getFtpClient").msg("ftp disconnects failure.").toString(), e);
